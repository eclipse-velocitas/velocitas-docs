---
title: "Tutorial to Deploy your Python Vehicle App with Helm"
...
This will be adapted to deploy a a new vehicle app, which is called `my_vehicle_app` for this walkthrough.
...
- A local K3D installation needs to be available. For how to setup K3D, check out this [tutorial](/run_runtime_services_kubernetes.md).
...
This script builds the local source code of the application into a container, pushes that container to the local cluster registry and deploys the app via a helm chart to the K3D cluster. Rerun this script after you changed the source code of your application to re-deploy with the latest changes.
